...
- June 20, 2025: Updated Season Highlights to show only stats from active club (Sporthood FC)
  - Changed hero banner to use `/api/stats/summary` for club-specific statistics instead of all-time data
  - Season Highlights now correctly displays performance only from Sporthood FC games
  - Updated attendance statistics on training page to only count sessions up to today (not future)
  - Added session completion progress tracking separate from attendance rate
  - Fixed training session attendance filtering using IST timezone for accurate date comparison
